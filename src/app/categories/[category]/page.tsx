...
import { selectCategoryBySlug, selectProductsByCategory } from "../../../redux/store/catalogSelectors"
    ...
    <main className="px-5 md:px-[10%] pb-16">
      <section className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#2a1e14]/60 backdrop-blur-sm mt-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center p-6 md:p-10">
          <div>
            <p className="text-sm uppercase tracking-[0.25em] text-[#d9a066]">Category</p>
            ...
            {cat.description && <p className="mt-3 text-[#f5e6d3]/80">{cat.description}</p>}
          </div>
      ...
      {cat.subcategories.length > 0 && (
        <section className="mt-8">
          <h2 className="text-xl font-bold text-[#f5e6d3] mb-3">Explore {cat.name}</h2>
            ...
            {cat.subcategories.map((s) => (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  {s.description && <p className="text-sm text-[#e2c9a6]">{s.description}</p>}
                </div>
              </a>
            ))}
          </div>
      ...
      <section className="mt-8">
        <h2 className="text-xl font-bold text-[#f5e6d3] mb-3">All {cat.name}</h2>
